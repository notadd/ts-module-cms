...
export class ArticleService {
    ...
    async reductionClassity(id: number, limit?: number, pages?: number) {
        ...
        let array: number[] = await this.classifyService.getClassifyId(id).then(a => {
            return a
        });
        array.push(id);
    ...
    async getLevelByClassifyId(id: number): Promise<string> {
        ...
        let num: number = await this.classifyService.findLevel(entity.id).then(a => {
            return a
        });
        let level: string = this.classifyService.interfaceChange(num);
        let topPlace: string = '';
    ...
    async upLoadPicture(req: any, bucketName: string, rawName: string, base64: string, id?: number) {
        try {
            ...
            //上传图片
            let result = await this.storeService.upload(bucketName, rawName, base64, imagePreProcessInfo).then(a => {
                return a
            });
            ...
            //获取图片地址
            let url = await this.storeService.getUrl(req.get('obj'), bucket, name, type, imagePreProcessInfo).then(a => {
                return a
            });
