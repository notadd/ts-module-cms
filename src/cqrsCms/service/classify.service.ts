...
export class ClassifyService {
    ...
    async createClassifyArt(entity: ClassifyEntity, limit?: number): Promise<ClassifyEntity[]> {
        ...
        if (firstClass.length == 0) {
            ...
            newClassify.title = '无';
            let id = await this.repository.createQueryBuilder().insert().into(ClassifyEntity).values(newClassify).output('id').execute().then(a => {
                return a
            });
    ...
    async createClassifyPage(entity: PageClassifyEntity, limit?: number): Promise<PageClassifyEntity[]> {
        ...
        if (firstClass.length == 0) {
            ...
            newClassify.classifyAlias = '无';
            newClassify.title = '无';
            let id = await this.pageRepository.createQueryBuilder().insert().into(PageClassifyEntity).values(newClassify).output('id').execute().then(a => {
                return a
            });
            let str: string = JSON.stringify(id).split(':')[ 1 ];
            let numb: string = str.substring(0, str.lastIndexOf('}'));
    ...
    async deleteMethodFirst(id: number) {
        ...
        let array: number[] = await this.getClassifyId(id).then(a => {
            return a
        });
        array.push(id);
    ...
    async deleteMethodSecond(id: number): Promise<PageClassifyEntity[]> {
        ...
        let array: number[] = await this.getClassifyIdPage(id).then(a => {
            return a
        });
        array.push(id);
        let newArray: number[] = Array.from(new Set(array));
    ...
    async showBeforeTitle(id: number): Promise<ArticleEntity[]> {
        ...
        let articles: ArticleEntity[] = [];
        let currentArticle: ArticleEntity[] = await this.artRepository.createQueryBuilder().where('"classifyId"= :classifyId and "topPlace"=\'current\'', { classifyId: classify.groupId }).orderBy('"updateAt"', 'ASC').getMany();
        articles.push(...currentArticle);
        let array: number[] = await this.getClassifyId(classify.groupId).then(a => {
            return a
        });
        array.push(id);
        let newArray: number[] = Array.from(new Set(array));
    ...
    public async findLevel(id: number): Promise<number> {
        ...
        let final: ClassifyEntity[] = await this.showClassifyLevel(arr, id, 0).then(a => {
            return a
        });
        let num: number;
        for (let t in final) {
    ...
    async classifyTopPlace(id: number, display?: number[]) {
        ...
        for (let t in display) {
            let array: number[] = await this.getClassifyId(display[ t ]).then(a => {
                return a
            });
            let newArray: number[] = Array.from(new Set(array));
        ...
        if (deleteNum > 0) {
            if (useFor == 'art') {
                ...
                if (entity == null) {
                    ...
                    let array: number[] = await this.getClassifyId(deleteNum).then(a => {
                        return a
                    });
                    let newArray: number[] = Array.from(new Set(array));
                    let artiicles: ArticleEntity[] = await this.artRepository.createQueryBuilder().where('"classifyId" in (:id)', { id: newArray }).getMany();
                ...
                if (entity == null) {
                    ...
                    let array: number[] = await this.getClassifyIdPage(deleteNum).then(a => {
                        return a
                    });
                    let newArray: number[] = Array.from(new Set(array));
                    let artiicles: PageEntity[] = await this.repositoryPage.createQueryBuilder().where('"classifyId" in (:id)', { id: newArray }).getMany();
